...
- **`0`, `false` and `0.0` rendered as blank cells, indistinguishable from NULL, in the default
  ...
  as nothing at all, reading as "no value" rather than the value it was. This affected
  `execute_query`, `list_tables`, `describe_table` and `get_table_data` — the primary text output
  — and was found by smoke-testing a foreign-key count that came back empty when `COUNT(*)` can
  never be NULL. Cells now use `??`, so only `null` and `undefined` collapse to empty.
  ...
  this; `test/unit/result-formatter.test.js` adds seven, three of which fail against the old code.
