...
/**
 * Categorize a bottleneck by average CPU time (microseconds) into a severity.
 * @param {number} avgCpuUs - average CPU time in microseconds
 * @returns {'CRITICAL'|'HIGH'|'MEDIUM'|'LOW'}
 */
function classifyBottleneckSeverity(avgCpuUs) {
...
/**
 * Classify a bottleneck row as IO-bound, CPU-bound, or normal.
 * @param {object} row - DMV row with avg_logical_reads and avg_cpu_time_us
 * @returns {'IO_BOUND'|'CPU_BOUND'|'NORMAL'}
 */
function classifyBottleneckType(row) {
  if (row.avg_logical_reads >= 100000) return 'IO_BOUND';
  if (row.avg_cpu_time_us / 1000 >= 1000) return 'CPU_BOUND';
  return 'NORMAL';
}

/** Classify a recognized primary wait, or return null to use metric analysis. */
function classifyWaitBottleneck(detector, primaryWaitType, physicalReads) {
  if (detector.isWaitType(primaryWaitType, 'io')) {
    return physicalReads > detector.config.thresholds.highPhysicalReads
      ? 'IO_INTENSIVE'
      : 'IO_MODERATE';
  }
  if (detector.isWaitType(primaryWaitType, 'cpu')) return 'CPU_INTENSIVE';
  if (detector.isWaitType(primaryWaitType, 'memory')) return 'MEMORY_PRESSURE';
  if (detector.isWaitType(primaryWaitType, 'locking')) return 'BLOCKING_LOCKS';
  if (detector.isWaitType(primaryWaitType, 'network')) return 'NETWORK_BOTTLENECK';
  return null;
}

/** Classify captured metrics in priority order, retaining the original ratio semantics. */
function classifyMetricBottleneck(
  detector,
  { avg_duration_ms, avg_cpu_time_ms, avg_logical_reads, avg_physical_reads, avg_wait_time_ms }
) {
  const cpuRatio = avg_cpu_time_ms / avg_duration_ms;
  const waitRatio = avg_wait_time_ms / avg_duration_ms;

  if (avg_physical_reads > detector.config.thresholds.highPhysicalReads) return 'IO_INTENSIVE';
  if (cpuRatio > 0.7) return 'CPU_INTENSIVE';
  if (waitRatio > 0.5) return 'WAIT_INTENSIVE';
  if (avg_logical_reads > detector.config.thresholds.highLogicalReads) return 'MEMORY_INTENSIVE';
  return 'GENERAL_SLOW';
}
...
export class BottleneckDetector {
  ...
  identifyBottleneckType(queryData) {
    ...
    if (wait_stats.length > 0) {
      ...
      const waitBottleneck = classifyWaitBottleneck(this, primaryWaitType, avg_physical_reads);
      if (waitBottleneck !== null) return waitBottleneck;
    }

    // Use the values captured before wait selection; do not re-read queryData properties.
    return classifyMetricBottleneck(this, {
      avg_duration_ms,
      avg_cpu_time_ms,
      avg_logical_reads,
      avg_physical_reads,
  ...
  /**
   * Detects the most expensive queries in a database via query-stats DMVs.
   * @param {string} database - Database name to analyze (optional)
   * @param {object} [options]
   * @param {number} [options.limit=10] - Max number of bottlenecks to return
   * @param {string|null} [options.severityFilter=null] - LOW|MEDIUM|HIGH|CRITICAL (case-insensitive)
   * @returns {Promise<Array<object>>} Bottleneck objects ranked by total CPU time
   */
  async detectBottlenecks(database, { limit = 10, severityFilter = null } = {}) {
    ...
    const bottlenecks = (result.recordset || []).map(r => ({
      query: r.query_text,
      severity: classifyBottleneckSeverity(r.avg_cpu_time_us),
      ...
      type: classifyBottleneckType(r)
    }));

    if (!severityFilter) {
      return bottlenecks;
    }
    const wantedSeverity = String(severityFilter).toUpperCase();
    return bottlenecks.filter(b => b.severity === wantedSeverity);
  }
}
