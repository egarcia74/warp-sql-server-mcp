...
function indexJoinConditions(query) {
  ...
  for (let i = length - 1; i >= 0; i--) {
    ...
    let suffixEnd = i + 1 === length ? length : 0;
    if (whitespaceEnd[i + 1] > i + 1) {
      const keywordStart = whitespaceEnd[i + 1];
      const keyword = /^(?:WHERE|GROUP|ORDER)/i.exec(query.slice(keywordStart, keywordStart + 5));
      if (keyword) suffixEnd = keywordStart + keyword[0].length;
    }
    if (!/[\n\r\u2028\u2029]/.test(query[i])) {
      bodyEnd[i] = suffixEnd || bodyEnd[i + 1];
    }
  }
...
export class QueryOptimizer {
  ...
  extractJoinColumns(query) {
    ...
    while ((join = joins.exec(query)) !== null) {
      ...
      const columnMatches = query
        .slice(start, end)
        .match(/\b([a-zA-Z_][a-zA-Z0-9_]*\.[a-zA-Z_][a-zA-Z0-9_]*)\b/g);
