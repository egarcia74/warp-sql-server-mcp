...
export class QueryOptimizer {
  ...
  extractWhereColumns(query) {
    ...
    // Extract column names - properly handle all comparison operators including multi-character ones
    const columnMatches = whereClause.match(/\b([a-zA-Z_][a-zA-Z0-9_]*)\s*(?:>=|<=|!=|<>|[=<>!])/g);
    if (columnMatches) {
      columnMatches.forEach(match => {
        // Remove all comparison operators (single and multi-character) and whitespace
