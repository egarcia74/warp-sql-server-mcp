...
const IDENTIFIER_PART = new RegExp(IDENTIFIER_PART_SOURCE, 'g');
// Anchored at the text following the FROM keyword: up to four dot-separated parts.
const TARGET_TABLE_PATTERN = new RegExp(
  String.raw`^\s*((?:${IDENTIFIER_PART_SOURCE})(?:\s*\.\s*(?:${IDENTIFIER_PART_SOURCE})){0,3})`
);
 ...
 * approved escaper, e.g. `dbo.Users` -> `[dbo].[Users]`.
 * @param {string} rawName - object name as written in the query
 * @returns {string} bracket-quoted, escaped object name
 */
function formatQualifiedTable(rawName) {
  const parts = [...rawName.matchAll(IDENTIFIER_PART)].map(m => m[0]);
  return parts.map(p => `[${escapeBracketIdentifier(unquoteIdentifierPart(p))}]`).join('.');
}
...
export class QueryOptimizer {
  ...
  generateIndexRecommendations(query, stats, _planData) {
    ...
    if (whereColumns.length > 0) {
      ...
      suggestions.push({
        ...
        ...this.buildCreateIndexSuggestion(
    ...
    if (upperQuery.includes('ORDER BY')) {
      ...
      if (orderByColumns.length > 0) {
        suggestions.push({
          type: 'INDEX_RECOMMENDATION',
          priority: 'MEDIUM',
          ...this.buildCreateIndexSuggestion('IX_OrderBy', targetTable, orderByColumns),
  ...
  /**
   * Builds the CREATE INDEX text shared by the WHERE and ORDER BY
   * recommendations. When the target table is known the statement is valid
   * T-SQL naming that table; otherwise the suggestion is explicitly labelled
   * conceptual instead of carrying a fake `[table]` identifier (#1102).
   * @param {string} indexName - index name to emit
   * @param {string|null} targetTable - bracket-quoted table from extractTargetTable
   * @param {string[]} columns - key columns
   * @returns {{suggestion: string, table?: string, conceptual?: true}} fields to merge
   */
  buildCreateIndexSuggestion(indexName, targetTable, columns) {
    const columnList = columns.join(', ');
    if (targetTable) {
      const executable = {
        suggestion: `CREATE INDEX ${indexName} ON ${targetTable} (${columnList})`,
        table: targetTable
      };
      return executable;
    }
    const conceptual = {
      suggestion:
        'Conceptual (target table could not be determined from the query): ' +
  ...
  /**
   * Resolves the primary target table from the query's first FROM clause as a
   * bracket-quoted, schema-qualified identifier (e.g.
   * `[Imports].[PR_Debtors_P1DEBACCT]`) suitable for CREATE INDEX. Returns null
   * when the first FROM does not name a table (derived table, table-valued
   * function, no FROM at all) so callers can label suggestions conceptual.
   * @param {string} query - SQL query
  ...
  extractTargetTable(query) {
    ...
    const tableMatch = TARGET_TABLE_PATTERN.exec(afterFrom);
    return tableMatch ? formatQualifiedTable(tableMatch[1]) : null;
  }
