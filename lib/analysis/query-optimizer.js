...
// Regular (unquoted) T-SQL identifier: Unicode letters/_/@/# first, then
// letters, digits, combining marks and _/$/@/#.
const REGULAR_IDENTIFIER_SOURCE = String.raw`[\p{L}_@#][\p{L}\p{N}\p{M}_$@#]*`;
// One part of a (possibly multi-part) object name: a bracket-delimited
// identifier (with `]]` escapes), a double-quoted identifier, or a regular one.
const IDENTIFIER_PART_SOURCE = String.raw`\[(?:[^\]]|\]\])+\]|"[^"]+"|${REGULAR_IDENTIFIER_SOURCE}`;
...
const LEADING_IDENTIFIER = new RegExp(String.raw`^(?:${IDENTIFIER_PART_SOURCE})`, 'u');
const QUALIFIED_NAME_TOKENS = new RegExp(String.raw`${IDENTIFIER_PART_SOURCE}|\.`, 'gu');
...
const IDENTIFIER_CHAR = /[\p{L}\p{N}\p{M}_$@#]/u;
// Characters that may legally follow a complete identifier. Anything else means
// the identifier grammar did not cover the whole token, so the name would be
// truncated — refuse to guess in that case.
...
const COMMENT_FILL = ' ';
const TOKEN_FILL = '~';
// On the mask a column reference is regular-identifier parts and/or runs of
// TOKEN_FILL (masked bracketed/quoted parts) joined by dots, not preceded by an
// identifier character, a dot or a placeholder.
const MASK_NAME_PART_SOURCE = String.raw`(?:~+|${REGULAR_IDENTIFIER_SOURCE})`;
const WHERE_COLUMN_PATTERN = new RegExp(
  String.raw`(?<![\p{L}\p{N}\p{M}_$@#.~])(${MASK_NAME_PART_SOURCE}(?:\.${MASK_NAME_PART_SOURCE}){0,3})\s*(>=|<=|!=|<>|=|>|<|!)`,
  'gu'
);
// WHERE columns are reported grouped by comparison operator in this order (the
// contract of the original extractor, which existing callers rely on).
const WHERE_OPERATOR_RANK = ['>=', '<=', '!=', '<>', '=', '>', '<', '!'];
// The lookbehind forbids starting a match inside a whitespace run. It changes
// nothing about what is matched: if a match begins at position p and p-1 is
// also whitespace, a match begins at p-1 too and is the one found first, so the
// leftmost match always starts where the run starts and exec().index and [0]
// are as before. What it removes is the failed attempt from every interior
// position, each scanning the rest of the run before giving up - quadratic on
// an item like `a<9,970 spaces>b` (Sonar javascript:S8786). Input is MCP-client
// SQL capped at MAX_ANALYZABLE_QUERY_LENGTH, so the old cost was bounded at
// ~0.3 s per call: robustness, not a vulnerability.
const SORT_DIRECTION_SUFFIX = /(?<!\s)\s+(?:asc|desc)\s*$/i;
const FROM_KEYWORD = /^from\b/i;
const WITH_KEYWORD = /^with\b/i;
const AS_KEYWORD = /^as\b/i;
const WHERE_KEYWORD = /^where\b/i;
const ORDER_BY_KEYWORD = /^order\s+by\b/i;
...
// Clauses that end the top-level WHERE clause of the first query block.
const WHERE_CLAUSE_TERMINATOR = /^(?:group|having|order|option|for|union|except|intersect)\b/i;
// Clauses that end the top-level ORDER BY list (LIMIT kept for legacy inputs).
const ORDER_BY_TERMINATOR = /^(?:limit|offset|fetch|for|option)\b/i;
const SET_OPERATOR = /^(?:union|except|intersect)\b/i;
// Words that can follow a table source but are never its alias.
const NOT_AN_ALIAS =
  /^(?:with|tablesample|inner|left|right|full|cross|outer|on|join|apply|pivot|unpivot|where|group|having|order|option|for|union|except|intersect)\b/i;
...
// A parenthesised group made only of legacy table hints, e.g. `(NOLOCK)`, which
// is not a function call.
const TABLE_HINT_GROUP =
  /^\(\s*(?:(?:nolock|readuncommitted|readcommitted|repeatableread|serializable|holdlock|updlock|xlock|rowlock|paglock|tablock|tablockx|readpast|nowait|index\s*\([^()]*\))(?:\s*,\s*|\s+)?)+\)/iu;
...
/** Return a JOIN condition's suffix end at this position, or zero if absent. */
function joinSuffixEnd(query, position, whitespaceEnd) {
  if (position === query.length) return position;
  const keywordStart = whitespaceEnd[position];
  if (keywordStart === position) return 0;
  const keyword = /^(?:WHERE|GROUP|ORDER)/i.exec(query.slice(keywordStart, keywordStart + 5));
  return keyword ? keywordStart + keyword[0].length : 0;
}

/**
 * Index the legacy JOIN grammar in linear time. Each bodyEnd is the earliest
 * ending of `.+?` followed by whitespace + WHERE/GROUP/ORDER or true end.
 * Dot cannot cross a line break, but the terminating whitespace can.
 */
function indexJoinConditions(query) {
  const length = query.length;
  const whitespaceEnd = new Uint32Array(length + 1);
  const wordEnd = new Uint32Array(length + 1);
  const bodyEnd = new Uint32Array(length + 1);
  const previousBody = new Int32Array(length + 1);
  whitespaceEnd[length] = length;
  wordEnd[length] = length;

  for (let i = length - 1; i >= 0; i--) {
    whitespaceEnd[i] = /\s/.test(query[i]) ? whitespaceEnd[i + 1] : i;
    wordEnd[i] = /\w/.test(query[i]) ? wordEnd[i + 1] : i;
    if (!/[\n\r\u2028\u2029]/.test(query[i])) {
      bodyEnd[i] = joinSuffixEnd(query, i + 1, whitespaceEnd) || bodyEnd[i + 1];
    }
  }

  let previous = -1;
  for (let i = 0; i <= length; i++) {
    if (bodyEnd[i]) previous = i;
    previousBody[i] = previous;
  }
  return { whitespaceEnd, wordEnd, bodyEnd, previousBody };
}

/** Return the matched JOIN end, or zero when this candidate does not match. */
function findJoinEnd(query, start, { whitespaceEnd, wordEnd, bodyEnd, previousBody }) {
  let position = start + 4;
  // Require whitespace, table, whitespace, alias, whitespace. The old grammar
  // accepts only ASCII word characters and has no boundary before JOIN.
  for (const ends of [whitespaceEnd, wordEnd, whitespaceEnd, wordEnd, whitespaceEnd]) {
    if (ends[position] === position) return 0;
    position = ends[position];
  }
  if (!/^ON/i.test(query.slice(position, position + 2))) return 0;
  const whitespaceStart = position + 2;
  const bodyStart = previousBody[whitespaceEnd[whitespaceStart]];
  // Greedy ON whitespace may give characters back to the body, but must keep
  // at least one. The lookup avoids retrying a long suffix for every split.
  return bodyStart > whitespaceStart ? bodyEnd[bodyStart] : 0;
}

/** Preserve the last ON on the first line containing ON + whitespace. */
function joinColumnStart(query, start, end, whitespaceEnd) {
  let columnStart = start;
  let foundOn = false;
  for (let i = start; i < end; i++) {
    if (/[\n\r\u2028\u2029]/.test(query[i]) && foundOn) break;
    if (/^ON/i.test(query.slice(i, i + 2)) && whitespaceEnd[i + 2] > i + 2) {
      columnStart = whitespaceEnd[i + 2];
      foundOn = true;
    }
  }
  return columnStart;
...
/**
 * Index of the last character of the line comment opened at `openIndex`. CR,
 * LF and CRLF all end the comment; the line break itself is kept so line
...
function indexOfLineCommentEnd(sql, openIndex) {
  for (let i = openIndex + 2; i < sql.length; i++) {
    if (sql[i] === '\n' || sql[i] === '\r') {
      return i - 1;
    }
  }
 ...
 * keywords while every index maps 1:1 onto the original text. Returns null when
 * a literal, identifier or block comment is unterminated (malformed input) so
 * the caller fails to "unknown" instead of guessing (#1102).
 * @param {string} sql - full query text
...
/**
 * Run `analyze(mask)` over the lexical mask of `query`, or return `fallback`
 * when the query is not analyzable (non-string, oversized or malformed).
 * @param {*} query - candidate query text
 * @param {(mask: string) => *} analyze - mask consumer
 * @param {*} fallback - value for unanalyzable input
 * @returns {*} analysis result or fallback
 */
function withMask(query, analyze, fallback) {
  if (!isAnalyzableQuery(query)) {
    return fallback;
  }
  const mask = maskSqlLiteralsAndComments(query);
  return mask === null ? fallback : analyze(mask);
}
 ...
 * @returns {number} length of the keyword found at `i` as a whole word, or 0
 */
function keywordLengthAt(mask, i, keyword) {
  if (!isWordStartAt(mask, i)) {
    return 0;
  }
  const match = keyword.exec(mask.slice(i, i + 32));
...
function keywordAt(mask, i, keyword) {
  return keywordLengthAt(mask, i, keyword) > 0;
}
 ...
 * @param {number} start - index to scan from
 * @param {RegExp} keyword - anchored `^keyword\b` pattern
 * @returns {number} first depth-0 index of the keyword, or -1
 */
function findTopLevelKeyword(mask, start, keyword) {
  return findTopLevel(mask, start, (m, i) => keywordAt(m, i, keyword));
}
...
/**
 * @param {string} mask - lexical mask
 * @param {number} i - index just past a token
 * @returns {boolean} whether the token is complete (followed by a separator or the end)
 */
function isSeparatorAt(mask, i) {
  return i >= mask.length || SEPARATOR_CHAR.test(mask[i]);
}
...
/**
 * Case-insensitive comparison key of a (partial) multi-part name:
 * `[Dbo].T` -> 'dbo.t'.
 * @param {string[]} parts - name parts
 * @returns {string} normalised key
 */
function nameKey(parts) {
  return parts.map(p => (p === '' ? '' : unquoteIdentifierPart(p).toLowerCase())).join('.');
}
...
/**
 * Read the single identifier written at mask index `pos` (after blanks) from
 * the original text. Returns `[raw, end]` or null when there is none or it is
 * not followed by a separator (truncated by the grammar).
 * @param {string} query - original query text
 * @param {string} mask - lexical mask of `query`
 * @param {number} pos - index to read from
 * @returns {[string, number]|null} identifier as written and end index, or null
 */
function readIdentifierAt(query, mask, pos) {
  ...
  return isSeparatorAt(mask, end) ? [match[0], end] : null;
}
 ...
 * name (derived table, VALUES, ...) or the name is not followed by a separator
 * (an unmodelled character would otherwise silently truncate it).
 * @param {string} query - original query text
 * @param {string} mask - lexical mask of `query`
...
function parseTableSource(query, mask, pos) {
  ...
  return parts && isSeparatorAt(mask, end) ? [parts, end] : null;
}
...
function isRowsetSource(mask, parts, end) {
  const next = skipBlank(mask, end);
  if (mask[next] === '(' && !TABLE_HINT_GROUP.test(mask.slice(next, next + 256))) {
    return true;
  }
...
/**
 * The alias of the single table source ending at `end` (`FROM t [AS] alias`),
 * lower-cased and unquoted; null when there is none or it cannot be read
 * safely.
 * @param {string} query - original query text
 * @param {string} mask - lexical mask of `query`
 * @param {number} end - mask index just past the table name
 * @returns {string|null} alias key, or null
 */
function parseAlias(query, mask, end) {
  let pos = skipBlank(mask, end);
  pos = skipBlank(mask, pos + keywordLengthAt(mask, pos, AS_KEYWORD));
  if (mask[pos] === '(' || keywordAt(mask, pos, NOT_AN_ALIAS)) {
    return null;
  }
  const identifier = readIdentifierAt(query, mask, pos);
  return identifier ? unquoteIdentifierPart(identifier[0]).toLowerCase() : null;
}
...
function collectCteNames(query, mask) {
  ...
  while (pos < mask.length) {
    ...
    names.add(unquoteIdentifierPart(identifier[0]).toLowerCase());
    const next = findTopLevel(mask, identifier[1], isCteListBoundary);
    if (next === -1 || mask[next] !== ',') {
      break;
    }
    pos = next + 1;
...
function isCteReference(query, mask, parts) {
  return parts.length === 1 && collectCteNames(query, mask).has(nameKey(parts));
}

/**
 * Resolve the query's single target table: the first depth-0 FROM's source
 * when it is a plain table (not a derived table, function/rowset source, CTE
 * or one of several sources). `qualifiers` are the comparison keys a column
 * qualifier may carry to belong to this table: its name, its full qualified
 * name and its alias, if any.
 * @param {string} query - original query text
 * @param {string} mask - lexical mask of `query`
 * @returns {{ table: string, qualifiers: Set<string> }|null} target, or null
 */
function resolveTargetSource(query, mask) {
  const fromIndex = findTopLevelKeyword(mask, 0, FROM_KEYWORD);
  if (fromIndex === -1) {
    return null;
  }
  ...
  if (
    isRowsetSource(mask, parts, end) ||
    hasMultipleTableSources(mask, end) ||
    isCteReference(query, mask, parts)
  ) {
    return null;
  }
  const qualifiers = new Set([nameKey(parts.slice(-1)), nameKey(parts)]);
  const alias = parseAlias(query, mask, end);
  if (alias) {
    qualifiers.add(alias);
  }
  const target = { table: formatQualifiedTable(parts), qualifiers };
  return target;
}

/**
 * Replace everything inside parentheses (and the parentheses themselves) with
 * the token placeholder, so scans of a clause never descend into subqueries,
 * EXISTS/IN lists or function arguments. Length is preserved.
 * @param {string} text - mask slice of one clause
 * @returns {string} flattened text
 */
function flattenParenthesised(text) {
  let depth = 0;
  let out = '';
...
/**
 * `[start, end)` mask range of the first depth-0 clause opened by `opener`,
 * ending at a depth-0 `terminator` keyword, `;` or the end of the query; null
 * when the clause is absent.
 * @param {string} mask - lexical mask
 * @param {RegExp} opener - anchored clause keyword pattern
 * @param {RegExp} terminator - anchored pattern of the keywords ending it
 * @returns {[number, number]|null} clause range, or null
 */
function topLevelClauseRange(mask, opener, terminator) {
  const openIndex = findTopLevelKeyword(mask, 0, opener);
  if (openIndex === -1) {
    return null;
  }
  const start = openIndex + keywordLengthAt(mask, openIndex, opener);
  const end = findTopLevel(mask, start, (m, i) => m[i] === ';' || keywordAt(m, i, terminator));
  return [start, end === -1 ? mask.length : end];
}

/**
 * Column references (as written) compared in the top-level WHERE clause of the
 * first query block, grouped by operator per WHERE_OPERATOR_RANK and
 * de-duplicated. Comments, literals and parenthesised text are never consulted.
 * @param {string} query - original query text
 * @param {string} mask - lexical mask of `query`
 * @returns {string[]} column tokens as written
 */
function extractTopLevelWhereTokens(query, mask) {
  const range = topLevelClauseRange(mask, WHERE_KEYWORD, WHERE_CLAUSE_TERMINATOR);
  if (!range) {
    return [];
  }
  const [start, end] = range;
  const region = flattenParenthesised(mask.slice(start, end));
  const ranked = [];
  for (const match of region.matchAll(WHERE_COLUMN_PATTERN)) {
    const tokenStart = start + match.index;
    const token = query.slice(tokenStart, tokenStart + match[1].length);
    if (PLAIN_COLUMN_PATTERN.test(token)) {
      ranked.push([WHERE_OPERATOR_RANK.indexOf(match[2]), token]);
    }
  }
  const ordered = ranked.sort((a, b) => a[0] - b[0]).map(entry => entry[1]);
  return [...new Set(ordered)];
}

/**
 * The ORDER BY item occupying `region[itemStart, itemEnd)`, read from the
 * original text without surrounding whitespace, a trailing sort direction or a
 * trailing terminator; null when empty.
 * @param {string} query - original query text
 * @param {string} region - flattened mask of the ORDER BY list
 * @param {number} offset - mask index where `region` starts
 * @param {number} itemStart - item start within `region`
 * @param {number} itemEnd - item end within `region`
 * @returns {string|null} item as written, or null
 */
function orderByItemToken(query, region, offset, itemStart, itemEnd) {
  const item = region.slice(itemStart, itemEnd);
  const direction = SORT_DIRECTION_SUFFIX.exec(item);
  const body = direction ? item.slice(0, direction.index) : item;
  const lead = body.length - body.trimStart().length;
  const trail = body.length - body.trimEnd().length;
  if (lead + trail >= body.length) {
    return null;
  }
  const token = stripColumnToken(
    query.slice(offset + itemStart + lead, offset + itemStart + body.length - trail)
  );
  return token.length > 0 ? token : null;
}

/**
 * ORDER BY items (as written) of the top-level ORDER BY list, without sort
 * directions. Parenthesised text is never descended into, so an ORDER BY
 * inside OVER(...) or a subquery is not an ORDER BY of the statement.
 * @param {string} query - original query text
 * @param {string} mask - lexical mask of `query`
 * @returns {string[]} items as written
 */
function extractTopLevelOrderByTokens(query, mask) {
  const range = topLevelClauseRange(mask, ORDER_BY_KEYWORD, ORDER_BY_TERMINATOR);
  if (!range) {
    return [];
  }
  const [start, end] = range;
  ...
  for (let i = 0; i <= region.length; i++) {
    if (i === region.length || region[i] === ',') {
      const token = orderByItemToken(query, region, start, itemStart, i);
      if (token) {
        tokens.push(token);
      }
      itemStart = i + 1;
...
/**
 * One lexical pass over the query for index recommendations: the resolved
 * target table, the top-level WHERE and ORDER BY tokens, and whether a set
 * operator (UNION/EXCEPT/INTERSECT) combines several query blocks — in which
 * case the ORDER BY applies to the combined result and cannot be attributed.
 * @param {string} query - SQL query
 * @returns {{ target: object|null, whereTokens: string[], orderByTokens: string[], setOperator: boolean }}
 */
function analyzeIndexStructure(query) {
  const empty = { target: null, whereTokens: [], orderByTokens: [], setOperator: false };
  return withMask(
    query,
    mask => {
      const structure = {
        target: resolveTargetSource(query, mask),
        whereTokens: extractTopLevelWhereTokens(query, mask),
        orderByTokens: extractTopLevelOrderByTokens(query, mask),
        setOperator: findTopLevelKeyword(mask, 0, SET_OPERATOR) !== -1
      };
      return structure;
    },
    empty
...
function stripColumnToken(token) {
  // Anchored like SORT_DIRECTION_SUFFIX: a start inside a `[\s;]` run can never be
  // the leftmost match, so forbidding it leaves the result unchanged and makes a
  // long interior run linear instead of quadratic (Sonar javascript:S8786).
  return token.trim().replace(/(?<![\s;])[\s;]+$/, '');
}

/**
 * Reduce ORDER BY / WHERE tokens to index key columns: ordinals and
 * expressions are dropped (they cannot be index keys), repeated columns are
 * listed once (a CREATE INDEX key list may not repeat a column), the table
 * qualifier is stripped from each plain column, and `attributable` is false when any
 * qualifier is not one of the target's accepted qualifiers (alias, table name)
 * — such a column belongs to another source and must not be attributed (#1102).
 * @param {string[]} tokens - cleaned clause tokens
 * @param {Set<string>|undefined} qualifiers - accepted qualifier keys of the target
 * @returns {{ columns: string[], written: string[], attributable: boolean }} key columns
 */
function toIndexKeyColumns(tokens, qualifiers) {
  const columns = [];
  const written = [];
  const seen = new Set();
  ...
  for (const token of tokens) {
    ...
    const key = nameKey(parts.slice(-1));
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
    ...
    if (parts.length > 1 && !qualifiers?.has(nameKey(parts.slice(0, -1)))) {
      attributable = false;
    }
  }
  const keys = { columns, written, attributable };
  return keys;
}

/**
 * Index name derived from the bare key columns (`IX_name_age`), with any
 * character that is not a letter, digit or underscore replaced by `_`.
 * @param {string[]} tokens - clause tokens
 * @returns {string} index name
 */
function indexNameForColumns(tokens) {
  const names = tokens
    .filter(token => PLAIN_COLUMN_PATTERN.test(token))
    .map(token => unquoteIdentifierPart(splitQualifiedName(token).at(-1)))
    .map(name => name.replace(/[^\p{L}\p{N}_]/gu, '_'));
  return `IX_${names.join('_')}`;
}
...
export class QueryOptimizer {
  ...
  generateIndexRecommendations(query, stats, _planData) {
    const suggestions = [];
    const structure = analyzeIndexStructure(query);

    // Analyze WHERE clauses for index opportunities
    const whereIndex = this.buildCreateIndexSuggestion(
      indexNameForColumns(structure.whereTokens),
      structure.target,
      structure.whereTokens
    );
    if (whereIndex) {
    ...
    // Analyze ORDER BY clauses. Over a set operation the ORDER BY sorts the
    // combined result, so it cannot be attributed to the first block's table.
    const orderByTarget = structure.setOperator ? null : structure.target;
    const orderByIndex = this.buildCreateIndexSuggestion(
      'IX_OrderBy',
      orderByTarget,
      structure.orderByTokens
    );
    if (orderByIndex) {
      suggestions.push({
        type: 'INDEX_RECOMMENDATION',
   ...
   * target table is known and every qualified column belongs to it (alias or
   * table name), the statement is valid T-SQL naming that table with bare
   * column names; otherwise the suggestion is explicitly labelled conceptual,
   * keeping the columns as written, instead of carrying a fake or wrong
   * identifier (#1102).
   * @param {string} indexName - index name to emit
   * @param {{ table: string, qualifiers: Set<string> }|null} target - resolved target
   ...
   */
  buildCreateIndexSuggestion(indexName, target, tokens) {
    const keys = toIndexKeyColumns(tokens, target?.qualifiers);
    if (keys.columns.length === 0) {
      return null;
    }
    if (target && keys.attributable) {
      const executable = {
        suggestion: `CREATE INDEX ${indexName} ON ${target.table} (${keys.columns.join(', ')})`,
        table: target.table
      };
      return executable;
    }
   ...
   * table-valued or rowset function, CTE name), the name is not fully covered
   * by the identifier grammar, or the FROM clause has more than one table
   * source (JOIN/APPLY/comma list) (#1102).
   * @param {string} query - SQL query
   * @returns {string|null} bracket-quoted table name, or null
   */
  extractTargetTable(query) {
    return withMask(query, mask => resolveTargetSource(query, mask)?.table ?? null, null);
  }
  ...
  /**
   * Extracts the column references compared in the top-level WHERE clause of
   * the first query block, as written (e.g. `t.name`), grouped by comparison
   * operator (>=, <=, !=, <>, =, >, <, !) and de-duplicated. The clause is
   * located on a lexical mask, so text inside comments, string literals and
   * parentheses (subqueries, EXISTS/IN lists) is never consulted (#1102).
   * Non-string, oversized (>10k chars) or malformed input yields [].
   * @param {string} query - SQL query
   * @returns {string[]} column references as written
   */
  extractWhereColumns(query) {
    return withMask(query, mask => extractTopLevelWhereTokens(query, mask), []);
  }
  ...
  extractJoinColumns(query) {
    ...
    const conditions = indexJoinConditions(query);
    const joins = /JOIN/gi;
    const columns = [];
    let join;
    while ((join = joins.exec(query)) !== null) {
      const end = findJoinEnd(query, join.index, conditions);
      // Failed candidates must not consume a possibly overlapping JOIN header.
      if (!end) continue;
      const start = joinColumnStart(query, join.index, end, conditions.whitespaceEnd);
      const columnMatches = query.slice(start, end).match(/\b([a-zA-Z_]\w*\.[a-zA-Z_]\w*)\b/g);
      if (columnMatches) columns.push(...columnMatches);
      joins.lastIndex = end;
    }
  ...
  /**
   * Extracts the items of the top-level ORDER BY list, as written and without
   * sort directions. The list is located on a lexical mask and ends at
   * LIMIT/OFFSET/FETCH/FOR/OPTION, `;` or the end of the query; parenthesised
   * text is never descended into, so an ORDER BY inside OVER(...) or a
   * subquery, a comment or a literal is not an ORDER BY of the statement
   * (#1102). Non-string, oversized or malformed input yields [].
   * @param {string} query - SQL query
   * @returns {string[]} ORDER BY items as written
   */
  extractOrderByColumns(query) {
    return withMask(query, mask => extractTopLevelOrderByTokens(query, mask), []);
  }
  ...
  /**
   * Describes how the caller's `analysis_period` was handled (#1103).
   *
   * The period is accepted but NOT applied. The two DMV sources have different
   * time semantics: the missing-index DMVs (sys.dm_db_missing_index_*) are
   * cumulative aggregates with no per-event history and cannot be windowed,
   ...
   * applied. Rather than silently returning identical output for every period,
   * the response says so explicitly.
   * @param {string|undefined} requested - The analysis_period the caller sent
   * @returns {{requested: (string|null), applied: boolean, reason: string}}
   */
  describeAnalysisPeriod(requested) {
    return {
      requested: requested ?? null,
      applied: false,
      reason:
        'reserved - the two DMV sources have different lifetimes and cannot be windowed consistently: ' +
        'sys.dm_db_missing_index_* aggregates are cumulative (reset only by a server restart or index/database changes), ' +
        'while sys.dm_exec_query_stats rows last only as long as their plan stays in cache; ' +
        "results reflect each source's own lifetime, not the requested period"
    };
  }
   ...
   *
   * `options.analysisPeriod` is accepted only so it can be echoed back in the
   * `analysisPeriod` disclosure (`applied: false`); see describeAnalysisPeriod
   * for why it is not applied.
   * @param {string} database - Database name
   * @param {object} [options]
   * @param {string} [options.analysisPeriod] - Requested period (24_HOURS, 7_DAYS, 30_DAYS)
  ...
  async getOptimizationInsights(database, options = {}) {
    ...
    return {
      database: database || null,
      timestamp: new Date().toISOString(),
      analysisPeriod: this.describeAnalysisPeriod(options.analysisPeriod),
      ...
      roadmap: this.buildInsightsRoadmap(highImpactIndexCount, expensiveQueryCount)
    };
  }

  /**
   * Builds the prioritized roadmap for getOptimizationInsights from the two
   * signal counts.
   * @param {number} highImpactIndexCount - High-priority missing indexes
   * @param {number} expensiveQueryCount - Queries over 1s average CPU
   * @returns {Array<{priority: string, action: string}>}
   */
  buildInsightsRoadmap(highImpactIndexCount, expensiveQueryCount) {
    const roadmap = [];
