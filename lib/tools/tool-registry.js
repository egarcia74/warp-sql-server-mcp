    ...
    inputSchema: {
      ...
      properties: {
        ...
        offset: {
          ...
          minimum: 0,
          description:
            'Number of rows to skip before returning results (optional, defaults to 0). Pair with limit to page through a table. Row order is not guaranteed without an ORDER BY, so pages may overlap or skip rows on tables without a clustered index.'
        },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        schema: {
          type: 'string',
          description:
            'Schema name to restrict recommendations to (optional; omit to cover all schemas)'
        },
        limit: {
          type: 'number',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        analysis_period: {
          type: 'string',
          description:
            'Analysis time period: 24_HOURS, 7_DAYS, 30_DAYS (optional). RESERVED: accepted but not yet applied. The missing-index DMVs are cumulative and cannot be windowed consistently, so results reflect the plan-cache lifetime regardless of the value sent; the response discloses this in its analysisPeriod field.',
