...
export class DatabaseToolsHandler extends BaseToolHandler {
  ...
  /**
   * Export table data as CSV format
   */
  async exportTableCsv(tableName, database = null, schema = 'dbo', limit = 1000) {
    let query;
    ...
    const result = await this.executeQuery(query, 'export_table_csv');
  ...
  async explainQuery(query, database = null) {
    ...
    try {
      const result = await this.executeQuery(explainQuery, 'explain_query');
      return this.formatResults(result);
    } catch {
      // If SHOWPLAN_ALL doesn't work, try with estimated execution plan
      const estimatedQuery = database
        ? `USE [${database}]; SET SHOWPLAN_TEXT ON; ${query}; SET SHOWPLAN_TEXT OFF;`
        : `SET SHOWPLAN_TEXT ON; ${query}; SET SHOWPLAN_TEXT OFF;`;

      try {
        const result = await this.executeQuery(estimatedQuery, 'explain_query');
        return this.formatResults(result);
      } catch (innerError) {
        return [
          {
