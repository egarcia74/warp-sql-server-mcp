...
export class DatabaseToolsHandler extends BaseToolHandler {
  ...
  formatExecutionPlan(result) {
    ...
    // executes the query and returns the plan in a separate recordset
    // alongside the data, so scan every recordset for the plan column.
    ...
    // No plan column found; fall back to a table of whatever was returned.
    return this.formatResults(result);
  }
}
