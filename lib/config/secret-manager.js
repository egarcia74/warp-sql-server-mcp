...
export class SecretManager {
  ...
  /**
   * Safely parse integer from secret value with validation
   * @param {string} secretValue - Secret value to parse
  ...
  /**
   * Validate AWS region to avoid unsafe/invalid values.
   *
   * AWS region inputs should be treated as untrusted configuration.
   * If a region is invalid, we fall back to a safe default.
   *
   * @param {string} region
   * @returns {string}
   * @private
  ...
  _validateAwsRegion(region) {
    ...
    if (!region || typeof region !== 'string') return fallback;

    const value = region.trim();
    if (!value) return fallback;

    // Conservative allow-list pattern for AWS regions.
    // Examples:
    // - Standard: us-east-1, ap-southeast-2, eu-central-1
    // - Gov: us-gov-west-1
    // - ISO/ISOB: us-iso-east-1, us-isob-east-1
    const isValid = /^(?:[a-z]{2}-(?:(?:gov|iso|isob)-)?[a-z]+-\d)$/.test(value);

    if (!isValid) {
      console.warn(`Invalid AWS region '${region}' provided. Falling back to '${fallback}'.`);
      return fallback;
    }
  ...
  initializeProviders() {
    if (this.secretSource === 'aws') {
      const region = this._validateAwsRegion(
      ...
      // Only set explicit static credentials when both keys are present;
      // otherwise defer to the AWS default credential provider chain
      // (IAM roles, SSO, ECS/EC2 metadata, etc.). Honor a session token
      // when supplied so temporary/STS credentials work as expected.
      ...
      this.awsSecretsClient = new SecretsManagerClient(clientConfig);
  ...
  async getFromAWS(secretName) {
    ...
    const result = await this.awsSecretsClient.send(new GetSecretValueCommand(params));
  ...
  async getDatabaseConfig() {
    const config = {
      ...
      port: this._safeParseInt(await this.getSecret('SQL_SERVER_PORT'), '1433', 1, 65535),
      ...
      connectionTimeout: this._safeParseInt(
        await this.getSecret('SQL_SERVER_CONNECT_TIMEOUT_MS'),
        '10000',
        1000,
        60000
      ),
      requestTimeout: this._safeParseInt(
        await this.getSecret('SQL_SERVER_REQUEST_TIMEOUT_MS'),
        '30000',
        5000,
        300000
      ),
      maxRetries: this._safeParseInt(await this.getSecret('SQL_SERVER_MAX_RETRIES'), '3', 0, 10),
      retryDelay: this._safeParseInt(
        await this.getSecret('SQL_SERVER_RETRY_DELAY_MS'),
        '1000',
        100,
        10000
      ),
      ...
      // Pool settings
      poolMax: this._safeParseInt(await this.getSecret('SQL_SERVER_POOL_MAX'), '10', 1, 100),
      poolMin: this._safeParseInt(await this.getSecret('SQL_SERVER_POOL_MIN'), '0', 0, 50),
      poolIdleTimeout: this._safeParseInt(
        await this.getSecret('SQL_SERVER_POOL_IDLE_TIMEOUT_MS'),
        '30000',
        1000,
        300000
      ),
      ...
      switch (this.secretSource) {
        ...
        case 'aws':
          if (this.awsSecretsClient) {
            ...
            try {
              await this.awsSecretsClient.send(new ListSecretsCommand({ MaxResults: 1 }));
              health.status = 'healthy';
