...
/** Append operation warnings in their existing display order. */
function collectOperationWarnings(config, warnings) {
  if (!config.readOnlyMode && config.allowDestructiveOperations) {
    warnings.push('Destructive operations are enabled - use caution in production');
  }
  if (!config.readOnlyMode && config.allowSchemaChanges) {
    warnings.push('Schema changes are enabled - use caution in production');
  }
}

/** Append SSL warnings without changing the security decision or its evaluation order. */
function collectSslWarnings(connectionSummary, securityDecision, warnings) {
...
/** Append numeric-limit errors, retaining the existing comparisons and field reads. */
function collectLimitErrors(config, errors) {
  ...
  if (
    config.performanceMonitoring.samplingRate < 0 ||
    config.performanceMonitoring.samplingRate > 1
  ) {
    errors.push('Performance sampling rate must be between 0 and 1');
  }
}

function appendRuntimeReport(configLines) {
...
function appendConnectionReport(configLines, connectionSummary) {
  configLines.push(
    '',
    '🌐 Connection Settings:',
...
function appendNetworkReport(configLines, config) {
  // Network and timing diagnostics
  configLines.push(
    '',
    '🔧 Network & Timing Diagnostics:',
    `    Connection String Length: ${config.getConnectionConfig().connectionString?.length || 0} chars`,
...
function appendSslReport(configLines, connectionSummary, connectionManager) {
...
function appendFeatureReport(configLines, config) {
  configLines.push(
    '',
    '🔒 Security & Operation Settings:',
    `    Debug Mode: ${config.debugMode}`,
    `    Read-Only Mode: ${config.readOnlyMode ? '🔒' : '🔓'} ${config.readOnlyMode}`,
    `    Allow Destructive Operations: ${config.allowDestructiveOperations ? '⚠️' : '✅'} ${config.allowDestructiveOperations}`,
    `    Allow Schema Changes: ${config.allowSchemaChanges ? '⚠️' : '✅'} ${config.allowSchemaChanges}`,
  ...
  if (config.performanceMonitoring.enabled) {
    configLines.push(
      `    Max History: ${config.performanceMonitoring.maxMetricsHistory} records`,
      `    Slow Query Threshold: ${config.performanceMonitoring.slowQueryThreshold}ms`,
      `    Track Pool Metrics: ${config.performanceMonitoring.trackPoolMetrics}`,
      `    Sampling Rate: ${config.performanceMonitoring.samplingRate * 100}%`
    );
  }
  configLines.push('', '📊 Streaming Configuration:', `    Enabled: ${config.streaming.enabled}`);
  if (config.streaming.enabled) {
    configLines.push(`    Batch Size: ${config.streaming.batchSize} rows`);
  }
  configLines.push(
    '',
    '📝 Logging & Output:',
    `    Log Level: ${config.logging.logLevel}`,
    `    Security Audit: ${config.logging.securityAudit}`
  );
}

function appendDependencyReport(configLines) {
...
function appendLogPathReport(configLines, config, logger) {
  ...
  if (logger?.config) {
    ...
    if (
      ...
      (config.logging.securityAudit && logDefaults.securityLogFile)
    ) {
...
function appendValidationReport(configLines, config) {
  configLines.push('====================================');
...
export class ServerConfig {
  ...
  validate() {
    ...
    collectOperationWarnings(this, warnings);
    const securityDecision = connectionSummary.securityDecision;
    collectSslWarnings(connectionSummary, securityDecision, warnings);
    collectLimitErrors(this, errors);
  ...
  logConfiguration(connectionManager = null, logger = null) {
    ...
    appendRuntimeReport(configLines);
    appendConnectionReport(configLines, connectionSummary);
    appendNetworkReport(configLines, this);
    appendSslReport(configLines, connectionSummary, connectionManager);
    appendFeatureReport(configLines, this);
    appendDependencyReport(configLines);
    appendLogPathReport(configLines, this, logger);
    appendValidationReport(configLines, this);

    const configMessage = configLines.join('\n');

    // Use logger if provided, otherwise fall back to console.error (stderr)
