...
export class ServerConfig {
   ...
   */
  logConfiguration(connectionManager = null) {
    if (process.env.NODE_ENV === 'test') return;

    const connectionSummary = this.getConnectionSummary();

    // Double-check password redaction for CodeQL security compliance
    // This ensures no raw password can ever be logged, even if getConnectionSummary() changes
    const safePassword =
      connectionSummary.password && connectionSummary.password.startsWith('***')
        ? connectionSummary.password
        : '***********'; // Fallback redaction if somehow unredacted
    ...
    console.error(`    Password: ${safePassword}`);
    ...
    if (connectionSummary.encrypt && connectionManager) {
      ...
      if (health.ssl) {
        console.error('');
        console.error('🔐 SSL Connection Information:');
        console.error(`    Status: ${health.ssl.connection_status}`);
        console.error(`    Protocol: ${health.ssl.protocol}`);
        console.error(`    Server: ${health.ssl.server}`);
        console.error(`    Encryption: ${health.ssl.encrypt ? 'Enabled' : 'Disabled'}`);
        console.error(
          `    Trust Server Certificate: ${health.ssl.trust_server_certificate ? 'Yes' : 'No'}`
        );
        if (health.ssl.note) {
          console.error(`    Note: ${health.ssl.note}`);
        }
      }
    }

    console.error('');
    console.error('🔒 Security & Operation Settings:');
    console.error(`    Debug Mode: ${this.debugMode}`);
    console.error(`    Read-Only Mode: ${this.readOnlyMode ? '🔒' : '🔓'} ${this.readOnlyMode}`);
    console.error(
    ...
    console.error(`    Enabled: ${this.performanceMonitoring.enabled}`);
    if (this.performanceMonitoring.enabled) {
      console.error(`    Max History: ${this.performanceMonitoring.maxMetricsHistory} records`);
      console.error(`    Slow Query Threshold: ${this.performanceMonitoring.slowQueryThreshold}ms`);
      console.error(`    Track Pool Metrics: ${this.performanceMonitoring.trackPoolMetrics}`);
      console.error(`    Sampling Rate: ${this.performanceMonitoring.samplingRate * 100}%`);
    }
    console.error('');
    console.error('📊 Streaming Configuration:');
    console.error(`    Enabled: ${this.streaming.enabled}`);
    if (this.streaming.enabled) {
      console.error(`    Batch Size: ${this.streaming.batchSize} rows`);
      console.error(`    Memory Limit: ${this.streaming.maxMemoryMB}MB`);
      console.error(
    ...
    console.error(`    Log Level: ${this.logging.logLevel}`);
    console.error(`    Security Audit: ${this.logging.securityAudit}`);
    console.error(`    Response Format: ${this.logging.responseFormat}`);
    console.error('====================================');

    // Log any configuration warnings/errors with emojis for visibility
    const validation = this.validate();
    if (validation.warnings.length > 0) {
      console.error('⚠️  Configuration Warnings:');
      validation.warnings.forEach(warning => console.error(`  ⚠️  ${warning}`));
    }
