...
export class PerformanceMonitor {
   ...
   * is the pool's capacity (`max`), `activeConnections` the connections
   * currently open (`size`), `idleConnections` those open and free
   * (`available`), `pendingRequests` the acquires waiting for a connection
   * (`pending`) and `borrowedConnections` the connections in use (`borrowed`).
  ...
  samplePoolStats() {
    ...
    const snapshot = {
      ...
      borrowedConnections: pool.borrowed
    };
