/**
 * Performance Monitoring System
 * Provides comprehensive performance metrics for query execution and connection pool management
 */

/**
 * Normalize an mssql `rowsAffected` value (an array with one entry per
 * statement) or an explicit integer to a single total. Returns undefined when
 * no usable value is present so the field can be omitted from the metric.
 * @param {number|number[]|undefined} rowsAffected - driver or explicit value
 * @returns {number|undefined} total rows affected, or undefined
 */
function normalizeRowsAffected(rowsAffected) {
  if (Array.isArray(rowsAffected)) {
    if (rowsAffected.length === 0) {
      return undefined;
    }
    return rowsAffected.reduce((sum, n) => sum + (Number.isInteger(n) ? n : 0), 0);
  }
  return Number.isInteger(rowsAffected) ? rowsAffected : undefined;
}

/**
 * Derive the returned-row count for a recorded query: an explicit `rowCount`
 * wins, then the driver `recordset` length, then the streaming `totalRows`
 * (the export_table_csv path). Returns undefined when none is available so a
 * misleading 0 is never reported (#1101).
 * @param {object} queryData - data passed to recordQuery
 * @returns {number|undefined} returned-row count, or undefined
 */
function resolveRowCount(queryData) {
  if (Number.isInteger(queryData.rowCount)) {
    return queryData.rowCount;
  }
  if (Array.isArray(queryData.recordset)) {
    return queryData.recordset.length;
  }
  return Number.isInteger(queryData.totalRows) ? queryData.totalRows : undefined;
}

/**
 * Copy the known row counts from `queryData` onto `target`, omitting any
 * count that is genuinely unavailable.
 * @param {object} target - metric (or plain object) receiving the counts
 * @param {object} queryData - data passed to recordQuery
 */
function applyRowCounts(target, queryData) {
  const rowsAffected = normalizeRowsAffected(queryData.rowsAffected);
  if (rowsAffected !== undefined) {
    target.rowsAffected = rowsAffected;
  }
  const rowCount = resolveRowCount(queryData);
  if (rowCount !== undefined) {
    target.rowCount = rowCount;
  }
}

/**
 * Shorten SQL text for storage in the metrics history.
 * @param {string|undefined} query - SQL text as recorded
 * @returns {string} the text, truncated to 200 characters, or 'unknown'
 */
function truncateQueryText(query) {
  if (!query) {
    return 'unknown';
  }
  return query.length > 200 ? `${query.substring(0, 200)}...` : query;
}

/**
 * @param {Date|undefined} timestamp - caller-supplied start timestamp
 * @returns {number} epoch milliseconds of the query start (now when absent)
 */
function resolveStartTime(timestamp) {
  return timestamp ? timestamp.getTime() : Date.now();
}

/**
 * Record the outcome of a tracked query on its metric: status/error, the
 * known row counts (same omit-don't-zero rule as recordQuery, #1101) and the
 * streaming flag.
 * @param {object} queryMetric - stored query metric
 * @param {object} result - driver result (or {} when none)
 * @param {Error|null} error - failure, if any
 */
function applyQueryOutcome(queryMetric, result, error) {
  queryMetric.status = error ? 'error' : 'completed';
  queryMetric.error = error ? error.message : null;
  applyRowCounts(queryMetric, result);
  queryMetric.streaming = result.streaming || false;
}

/**
 * Summarize a query metric for getQueryStats. `rowCount` and `rowsAffected`
 * are included only when they were actually recorded, so an unknown count is
 * omitted rather than shown as 0 or an undefined placeholder (#1101).
 * @param {object} q - stored query metric
 * @returns {object} summary row for the performance breakdown
 */
function summarizeQuery(q) {
  const summary = {
    tool: q.tool,
    duration: q.duration,
    status: q.status,
    streaming: q.streaming,
    timestamp: q.startTime
  };
  if (q.rowCount !== undefined) {
    summary.rowCount = q.rowCount;
  }
  if (q.rowsAffected !== undefined) {
    summary.rowsAffected = q.rowsAffected;
  }
  return summary;
}

/**
 * Extract the row counts from an mssql driver result in the shape recordQuery
 * accepts, so call sites can spread it into the metric they record:
 * `{ rowCount, rowsAffected }`, each omitted when unavailable (#1101).
 * @param {object|undefined} result - mssql query result
 * @returns {{ rowCount?: number, rowsAffected?: number }} known row counts
 */
export function extractRowCounts(result) {
  const counts = {};
  applyRowCounts(counts, { recordset: result?.recordset, rowsAffected: result?.rowsAffected });
  return counts;
}

export class PerformanceMonitor {
  constructor(config = {}) {
    this.config = {
      // Whether to enable performance monitoring
      enabled: config.enabled ?? true,
      // Maximum number of metrics to keep in memory
      maxMetricsHistory: config.maxMetricsHistory || 1000,
      // Slow query threshold in milliseconds
      slowQueryThreshold: config.slowQueryThreshold || 5000,
      // Whether to track detailed connection pool metrics
      trackPoolMetrics: config.trackPoolMetrics ?? true,
      // Sampling rate for performance metrics (0.0 to 1.0)
      samplingRate: config.samplingRate || 1.0,
      ...config
    };

    this.metrics = {
      queries: [],
      connections: [],
      poolHistory: [],
      poolStats: {
        totalConnections: 0,
        activeConnections: 0,
        idleConnections: 0,
        pendingRequests: 0,
        errors: 0
      },
      aggregates: {
        totalQueries: 0,
        slowQueries: 0,
        avgQueryTime: 0,
        maxQueryTime: 0,
        minQueryTime: Number.MAX_SAFE_INTEGER,
        totalQueryTime: 0,
        errorRate: 0
      }
    };

    this.startTime = Date.now();
    // Optional live source of connection-pool counters, see setPoolStatsSource()
    this.poolStatsSource = null;
  }

  /**
   * Attaches a source of live connection-pool counters. It is read once after
   * every recorded query (see samplePoolStats), which is what gives
   * TRACK_POOL_METRICS something to track: nothing else in production calls
   * recordPoolMetrics(). The callback is expected to return the shape of
   * ConnectionManager.getConnectionHealth(); anything without a usable
   * `pool.size` is ignored.
   * @param {(() => object|null|undefined)|null} source - callback, or null to detach
   */
  setPoolStatsSource(source) {
    this.poolStatsSource = typeof source === 'function' ? source : null;
  }

  /**
   * Takes one snapshot of the attached pool source and records it through
   * recordPoolMetrics(). The tarn counters are mapped onto the monitor's
   * fields so assessPoolHealth() reads them as intended: `totalConnections`
   * is the pool's capacity (`max`), `activeConnections` the connections in
   * use right now (`borrowed`), `idleConnections` those open and free
   * (`available`), `pendingRequests` the acquires waiting for a connection
   * (`pending`) and `openConnections` everything the pool currently holds
   * (`size` = borrowed + available + pending creates). `size` is deliberately
   * not "active": a pool that once grew to `max` keeps that size while idle,
   * so mapping it to active would read as "near capacity" forever.
   * A source that throws or returns no pool block records nothing - pool
   * sampling must never break query recording.
   */
  samplePoolStats() {
    if (!this.poolStatsSource || !this.config.enabled || !this.config.trackPoolMetrics) {
      return;
    }

    // Everything below is guarded: the counters are driver getters that can
    // throw, and several call sites invoke recordQuery() inside a try whose
    // catch records the query as failed and rethrows - an escaping error here
    // would turn a successful query into an error for the MCP client.
    try {
      const pool = this.poolStatsSource()?.pool;
      if (!pool || !Number.isInteger(pool.size)) {
        return;
      }

      const snapshot = {
        activeConnections: pool.borrowed,
        idleConnections: pool.available,
        pendingRequests: pool.pending,
        openConnections: pool.size
      };
      if (Number.isInteger(pool.max)) {
        snapshot.totalConnections = pool.max;
      }

      this.recordPoolMetrics(snapshot);
    } catch {
      // Pool sampling must never break query recording.
    }
  }

  /**
   * Records the start of a query execution
   * @param {string} toolName - Name of the MCP tool
   * @param {string} query - SQL query (truncated for performance)
   * @param {object} context - Execution context
   * @returns {string} Query ID for tracking
   */
  startQuery(toolName, query, context = {}) {
    if (!this.config.enabled || !this.shouldSample()) {
      return null;
    }

    const queryId = this.generateQueryId();
    const truncatedQuery = query.length > 200 ? `${query.substring(0, 200)}...` : query;

    const queryMetric = {
      id: queryId,
      tool: toolName,
      query: truncatedQuery,
      database: context.database || 'default',
      startTime: Date.now(),
      startMemory: this.getMemoryUsage(),
      status: 'running'
    };

    this.metrics.queries.push(queryMetric);
    this.trimMetricsHistory();

    return queryId;
  }

  /**
   * Records the completion of a query execution
   * @param {string} queryId - Query ID from startQuery
   * @param {object} result - Query execution result
   * @param {Error} error - Error if query failed
   */
  endQuery(queryId, result = {}, error = null) {
    if (!this.config.enabled || !queryId) {
      return;
    }

    const queryMetric = this.metrics.queries.find(q => q.id === queryId);
    if (!queryMetric) {
      return;
    }

    const endTime = Date.now();
    const duration = endTime - queryMetric.startTime;
    const endMemory = this.getMemoryUsage();

    // Update query metric
    queryMetric.endTime = endTime;
    queryMetric.duration = duration;
    queryMetric.endMemory = endMemory;
    queryMetric.memoryDelta = endMemory - queryMetric.startMemory;
    applyQueryOutcome(queryMetric, result ?? {}, error);

    // Update aggregates
    this.updateAggregates(queryMetric);

    // Log slow queries
    if (duration > this.config.slowQueryThreshold) {
      this.logSlowQuery(queryMetric);
    }
  }

  /**
   * Records connection pool metrics
   * @param {object} poolStats - Current pool statistics
   */
  recordPoolMetrics(poolStats) {
    if (!this.config.enabled || !this.config.trackPoolMetrics) {
      return;
    }

    const timestamp = Date.now();

    // Update current pool stats. Merge over the previous snapshot so a counter
    // the caller does not supply (e.g. `errors`, kept by recordConnectionEvent)
    // is carried forward rather than dropped.
    this.metrics.poolStats = {
      ...this.metrics.poolStats,
      ...poolStats,
      timestamp,
      uptime: timestamp - this.startTime
    };

    // Record the snapshot in its own history. It is not a connection *event*,
    // so it must not inflate totalConnectionEvents / recent.totalEvents.
    this.metrics.poolHistory.push({
      timestamp,
      ...poolStats
    });

    this.trimPoolHistory();
  }

  /**
   * Records connection events (connect, disconnect, error)
   * @param {string} event - Event type ('connect', 'disconnect', 'error', 'retry')
   * @param {object} details - Event details
   */
  recordConnectionEvent(event, details = {}) {
    if (!this.config.enabled) {
      return;
    }

    const connectionEvent = {
      id: this.generateEventId(),
      event,
      timestamp: Date.now(),
      ...details
    };

    this.metrics.connections.push(connectionEvent);
    this.trimConnectionHistory();

    // Update pool stats based on event
    switch (event) {
      case 'connect':
        this.metrics.poolStats.totalConnections++;
        this.metrics.poolStats.activeConnections++;
        break;
      case 'disconnect':
        this.metrics.poolStats.activeConnections--;
        break;
      case 'error':
        this.metrics.poolStats.errors++;
        break;
    }
  }

  /**
   * Gets current performance statistics
   * @returns {object} Performance statistics
   */
  getStats() {
    if (!this.config.enabled) {
      return { enabled: false };
    }

    const now = Date.now();
    const uptime = now - this.startTime;

    // Calculate recent performance (last 5 minutes). Include error-status
    // queries (matching getQueryStats and the overall aggregates) so the recent
    // window's count and errorRate reflect failures rather than always reporting
    // errorRate 0.
    const recentThreshold = now - 5 * 60 * 1000;
    const recentQueries = this.metrics.queries.filter(
      q => q.startTime > recentThreshold && (q.status === 'completed' || q.status === 'error')
    );

    const recentStats = this.calculateQueryStats(recentQueries);

    return {
      enabled: true,
      uptime,
      overall: this.metrics.aggregates,
      recent: recentStats,
      pool: this.metrics.poolStats,
      monitoring: {
        totalQueriesTracked: this.metrics.queries.length,
        totalConnectionEvents: this.metrics.connections.length,
        poolSnapshots: this.metrics.poolHistory.length,
        samplingRate: this.config.samplingRate,
        slowQueryThreshold: this.config.slowQueryThreshold
      }
    };
  }

  /**
   * Gets detailed query performance breakdown
   * @param {number} limit - Maximum number of queries to return
   * @param {object} [options] - Optional filters applied BEFORE the limit
   * @param {boolean} [options.slowOnly] - Only include queries slower than the
   *   configured slowQueryThreshold
   * @param {string} [options.toolFilter] - Only include queries for this MCP tool
   * @returns {object} Query performance details
   */
  getQueryStats(limit = 50, { slowOnly = false, toolFilter = null } = {}) {
    if (!this.config.enabled) {
      return { enabled: false };
    }

    // Apply the slow-only and tool-name filters BEFORE limiting so that `limit`
    // caps the filtered result set, not the raw history (otherwise a small limit
    // could exclude the very rows the filter is meant to surface).
    const completedQueries = this.metrics.queries
      .filter(q => q.status === 'completed' || q.status === 'error')
      .filter(q => (toolFilter ? q.tool === toolFilter : true))
      .filter(q => (slowOnly ? q.duration > this.config.slowQueryThreshold : true))
      .sort((a, b) => b.startTime - a.startTime)
      .slice(0, limit);

    // Group by tool
    const byTool = {};
    completedQueries.forEach(query => {
      if (!byTool[query.tool]) {
        byTool[query.tool] = {
          count: 0,
          totalTime: 0,
          errors: 0,
          slowQueries: 0
        };
      }

      byTool[query.tool].count++;
      byTool[query.tool].totalTime += query.duration;

      if (query.status === 'error') {
        byTool[query.tool].errors++;
      }

      if (query.duration > this.config.slowQueryThreshold) {
        byTool[query.tool].slowQueries++;
      }
    });

    // Calculate averages
    Object.keys(byTool).forEach(tool => {
      const stats = byTool[tool];
      stats.avgTime = stats.totalTime / stats.count;
      stats.errorRate = (stats.errors / stats.count) * 100;
      stats.slowQueryRate = (stats.slowQueries / stats.count) * 100;
    });

    return {
      enabled: true,
      queries: completedQueries.map(summarizeQuery),
      byTool,
      slowQueries: completedQueries.filter(q => q.duration > this.config.slowQueryThreshold)
    };
  }

  /**
   * Gets connection pool performance metrics
   * @returns {object} Connection pool metrics
   */
  getPoolStats() {
    if (!this.config.enabled || !this.config.trackPoolMetrics) {
      return { enabled: false };
    }

    const now = Date.now();
    const recentThreshold = now - 10 * 60 * 1000; // Last 10 minutes

    const recentConnections = this.metrics.connections.filter(
      conn => conn.timestamp > recentThreshold
    );

    // Calculate connection event rates
    const connectEvents = recentConnections.filter(c => c.event === 'connect').length;
    const errorEvents = recentConnections.filter(c => c.event === 'error').length;
    const retryEvents = recentConnections.filter(c => c.event === 'retry').length;

    return {
      enabled: true,
      current: this.metrics.poolStats,
      recent: {
        connectionRate: connectEvents / 10, // per minute
        errorRate: errorEvents / 10, // per minute
        retryRate: retryEvents / 10, // per minute
        totalEvents: recentConnections.length
      },
      health: this.assessPoolHealth()
    };
  }

  /**
   * Assesses connection pool health
   * @returns {object} Health assessment
   */
  assessPoolHealth() {
    const stats = this.metrics.poolStats;
    const issues = [];
    let status = 'healthy';

    // Check for high error rate
    if (stats.errors > 10) {
      issues.push('High error count detected');
      status = 'warning';
    }

    // Check for connection exhaustion (95% threshold to reduce false positives)
    if (stats.totalConnections > 0 && stats.activeConnections >= stats.totalConnections * 0.95) {
      issues.push('Connection pool near capacity');
      status = 'warning';
    }

    // Check for pending requests
    if (stats.pendingRequests > 5) {
      issues.push('High number of pending requests');
      if (status === 'healthy') status = 'warning';
    }

    // Critical: requests are waiting but nothing is idle and nothing is in
    // use, so the pool is not managing to create connections at all. A full
    // pool with requests waiting stays a warning (near capacity + pending).
    if (stats.pendingRequests > 0 && stats.idleConnections === 0 && stats.activeConnections === 0) {
      issues.push('Requests waiting with no connection available');
      status = 'critical';
    }

    return {
      status,
      issues,
      score: this.calculateHealthScore(stats, issues)
    };
  }

  /**
   * Calculates a health score (0-100)
   * @param {object} stats - Pool statistics
   * @param {array} issues - Identified issues
   * @returns {number} Health score
   */
  calculateHealthScore(stats, issues) {
    let score = 100;

    // Deduct points for issues
    score -= issues.length * 20;

    // Deduct points for high utilization
    if (stats.totalConnections > 0) {
      const utilization = stats.activeConnections / stats.totalConnections;
      if (utilization > 0.8) score -= 10;
      if (utilization > 0.9) score -= 10;
    }

    // Deduct points for errors
    if (stats.errors > 0) {
      score -= Math.min(stats.errors * 2, 30);
    }

    return Math.max(0, score);
  }

  /**
   * Updates aggregate statistics
   * @param {object} queryMetric - Completed query metric
   */
  updateAggregates(queryMetric) {
    const agg = this.metrics.aggregates;

    agg.totalQueries++;

    if (queryMetric.status === 'error') {
      agg.errorRate = (agg.errorRate * (agg.totalQueries - 1) + 1) / agg.totalQueries;
    } else {
      agg.errorRate = (agg.errorRate * (agg.totalQueries - 1)) / agg.totalQueries;
    }

    if (queryMetric.duration) {
      agg.totalQueryTime += queryMetric.duration;
      agg.avgQueryTime = agg.totalQueryTime / agg.totalQueries;
      agg.maxQueryTime = Math.max(agg.maxQueryTime, queryMetric.duration);
      agg.minQueryTime = Math.min(agg.minQueryTime, queryMetric.duration);

      if (queryMetric.duration > this.config.slowQueryThreshold) {
        agg.slowQueries++;
      }
    }
  }

  /**
   * Calculates statistics for a set of queries
   * @param {array} queries - Array of query metrics
   * @returns {object} Calculated statistics
   */
  calculateQueryStats(queries) {
    if (queries.length === 0) {
      return {
        count: 0,
        avgDuration: 0,
        maxDuration: 0,
        minDuration: 0,
        errorRate: 0,
        slowQueryRate: 0
      };
    }

    const durations = queries.map(q => q.duration).filter(d => d != null);
    const errors = queries.filter(q => q.status === 'error').length;
    const slowQueries = queries.filter(q => q.duration > this.config.slowQueryThreshold).length;

    return {
      count: queries.length,
      avgDuration: durations.reduce((a, b) => a + b, 0) / durations.length || 0,
      maxDuration: Math.max(...durations) || 0,
      minDuration: Math.min(...durations) || 0,
      errorRate: (errors / queries.length) * 100,
      slowQueryRate: (slowQueries / queries.length) * 100
    };
  }

  /**
   * Logs a slow query for investigation
   * @param {object} queryMetric - Slow query metric
   */
  logSlowQuery(queryMetric) {
    console.warn(`Slow query detected: ${queryMetric.duration}ms`, {
      tool: queryMetric.tool,
      query: queryMetric.query,
      database: queryMetric.database,
      duration: queryMetric.duration,
      rowCount: queryMetric.rowCount
    });
  }

  /**
   * Determines if this operation should be sampled
   * @returns {boolean} Whether to sample
   */
  shouldSample() {
    return Math.random() < this.config.samplingRate;
  }

  /**
   * Generates a unique query ID
   * @returns {string} Unique query ID
   */
  generateQueryId() {
    return `q_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }

  /**
   * Generates a unique event ID
   * @returns {string} Unique event ID
   */
  generateEventId() {
    return `e_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  }

  /**
   * Gets current memory usage
   * @returns {number} Memory usage in MB
   */
  getMemoryUsage() {
    if (typeof process !== 'undefined' && process.memoryUsage) {
      return process.memoryUsage().heapUsed / (1024 * 1024);
    }
    return 0;
  }

  /**
   * Trims metrics history to prevent memory leaks
   */
  trimMetricsHistory() {
    if (this.metrics.queries.length > this.config.maxMetricsHistory) {
      // Keep the most recent metrics and some slow queries for analysis
      const recentQueries = this.metrics.queries.slice(
        -Math.floor(this.config.maxMetricsHistory * 0.8)
      );
      const slowQueries = this.metrics.queries
        .filter(q => q.duration > this.config.slowQueryThreshold)
        .slice(-Math.floor(this.config.maxMetricsHistory * 0.2));

      this.metrics.queries = [...recentQueries, ...slowQueries]
        .sort((a, b) => a.startTime - b.startTime)
        .slice(-this.config.maxMetricsHistory);
    }
  }

  /**
   * Trims connection history to prevent memory leaks
   */
  trimConnectionHistory() {
    if (this.metrics.connections.length > this.config.maxMetricsHistory) {
      this.metrics.connections = this.metrics.connections.slice(-this.config.maxMetricsHistory);
    }
  }

  /**
   * Trims pool-snapshot history to prevent memory leaks
   */
  trimPoolHistory() {
    if (this.metrics.poolHistory.length > this.config.maxMetricsHistory) {
      this.metrics.poolHistory = this.metrics.poolHistory.slice(-this.config.maxMetricsHistory);
    }
  }

  /**
   * Resets all performance metrics
   */
  reset() {
    this.metrics = {
      queries: [],
      connections: [],
      poolHistory: [],
      poolStats: {
        totalConnections: 0,
        activeConnections: 0,
        idleConnections: 0,
        pendingRequests: 0,
        errors: 0
      },
      aggregates: {
        totalQueries: 0,
        slowQueries: 0,
        avgQueryTime: 0,
        maxQueryTime: 0,
        minQueryTime: Number.MAX_SAFE_INTEGER,
        totalQueryTime: 0,
        errorRate: 0
      }
    };
    this.startTime = Date.now();
  }

  /**
   * Updates performance monitoring configuration
   * @param {object} newConfig - New configuration options
   */
  updateConfig(newConfig) {
    this.config = { ...this.config, ...newConfig };
  }

  /**
   * Gets current configuration
   * @returns {object} Current configuration
   */
  getConfig() {
    return { ...this.config };
  }

  /**
   * Generates a performance report
   * @returns {object} Comprehensive performance report
   */
  generateReport() {
    const stats = this.getStats();
    const queryStats = this.getQueryStats();
    const poolStats = this.getPoolStats();

    return {
      timestamp: Date.now(),
      uptime: stats.uptime,
      summary: {
        totalQueries: stats.overall.totalQueries,
        avgQueryTime: Math.round(stats.overall.avgQueryTime),
        slowQueries: stats.overall.slowQueries,
        errorRate: Math.round(stats.overall.errorRate * 100) / 100,
        poolHealth: poolStats.health?.status || 'unknown'
      },
      detailed: {
        overall: stats.overall,
        recent: stats.recent,
        pool: poolStats,
        queries: queryStats
      },
      recommendations: this.generateRecommendations(stats, queryStats, poolStats)
    };
  }

  /**
   * Generates performance recommendations
   * @param {object} stats - Overall stats
   * @param {object} queryStats - Query statistics
   * @param {object} poolStats - Pool statistics
   * @returns {array} Array of recommendations
   */
  generateRecommendations(stats, queryStats, poolStats) {
    const recommendations = [];

    // Query performance recommendations
    if (stats.overall.avgQueryTime > 1000) {
      recommendations.push({
        type: 'performance',
        priority: 'high',
        message: 'Average query time is high. Consider query optimization or indexing.',
        metric: 'avgQueryTime',
        value: stats.overall.avgQueryTime
      });
    }

    if (stats.overall.errorRate > 5) {
      recommendations.push({
        type: 'reliability',
        priority: 'critical',
        message: 'High error rate detected. Check query validation and database connectivity.',
        metric: 'errorRate',
        value: stats.overall.errorRate
      });
    }

    // Pool recommendations
    if (poolStats.enabled && poolStats.health?.status === 'warning') {
      recommendations.push({
        type: 'infrastructure',
        priority: 'medium',
        message: 'Connection pool health issues detected.',
        issues: poolStats.health.issues
      });
    }

    // Tool-specific recommendations
    if (queryStats.enabled && queryStats.byTool) {
      Object.entries(queryStats.byTool).forEach(([tool, toolStats]) => {
        if (toolStats.avgTime > 2000) {
          recommendations.push({
            type: 'optimization',
            priority: 'medium',
            message: `Tool '${tool}' has high average execution time.`,
            metric: 'toolAvgTime',
            tool,
            value: toolStats.avgTime
          });
        }
      });
    }

    return recommendations;
  }

  /**
   * Simple recordQuery method for compatibility with existing code
   * @param {object} queryData - Query execution data
   */
  recordQuery(queryData) {
    // Same gate as startQuery(): PERFORMANCE_SAMPLING_RATE applies to every
    // production call site, all of which record through here (#1211).
    if (!this.config.enabled || !this.shouldSample()) {
      return;
    }

    const queryMetric = {
      id: this.generateQueryId(),
      tool: queryData.tool || 'unknown',
      query: truncateQueryText(queryData.query),
      database: queryData.database || 'default',
      startTime: resolveStartTime(queryData.timestamp),
      duration: queryData.executionTime || 0,
      status: queryData.success ? 'completed' : 'error',
      error: queryData.error || null
    };
    // Only record counts that are actually known — a hard-coded 0 misreported
    // every tool's row volume in get_query_performance (#1101).
    applyRowCounts(queryMetric, queryData);

    this.metrics.queries.push(queryMetric);
    this.trimMetricsHistory();
    this.updateAggregates(queryMetric);

    // Log slow queries
    if (queryMetric.duration > this.config.slowQueryThreshold) {
      this.logSlowQuery(queryMetric);
    }

    // One pool snapshot per recorded query, taken after the query so the
    // connection it used has been released back to the pool.
    this.samplePoolStats();
  }
}
