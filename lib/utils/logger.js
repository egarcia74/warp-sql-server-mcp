...
import { isMcpStdioTransport } from './mcp-environment.js';

/**
 * Every level routed to stderr when stdout is a protocol stream.
 *
 * Winston's Console transport maps a level to stderr only if it is named in
 * `stderrLevels`; an unset option sends everything to `console._stdout.write`. There is
 * no "all levels" shorthand, so the list is exhaustive over Winston's npm levels - a
 * level left out here is a level that would land in the JSON-RPC stream.
 */
const STDERR_LEVELS = Object.freeze(['error', 'warn', 'info', 'debug', 'verbose', 'silly']);
...
export class Logger {
  ...
  createLogger() {
    ...
    // Under a stdio transport, avoid colors: they show as raw ANSI codes in VS Code, and
    // the same judgement decides below that this output must not go to stdout.
    const isMcpEnv = this._isMcpStdioTransport();
    ...
    if (process.env.NODE_ENV !== 'test' || process.env.ENABLE_TEST_LOGGING === 'true') {
      ...
      // Under a stdio transport, send every level to stderr: stdout carries JSON-RPC.
      if (isMcpEnv) {
        consoleTransportOptions.stderrLevels = [...STDERR_LEVELS];
      }
  ...
  createSecurityLogger() {
    ...
    if (process.env.NODE_ENV !== 'test' || process.env.ENABLE_TEST_LOGGING === 'true') {
      ...
      // The audit channel gets the same treatment as the main logger rather than being
      // dropped under a stdio transport. An audit line that silently disappears is its
      // own hazard - with no SECURITY_LOG_FILE set, the console is the only record of a
      // blocked query - so the fix moves the record, it does not discard it.
      if (this._isMcpStdioTransport()) {
        consoleTransportOptions.stderrLevels = [...STDERR_LEVELS];
      }
  ...
  /**
   * Whether stdout is carrying an MCP stdio protocol stream, in which case console
   * output must be routed to stderr.
   *
   * Renamed from `_isMcpEnvironment()` in #1256, because the question it answers changed:
   * it used to ask "is the editor VS Code?" and now asks "is this the stdio transport?".
   * The old name invited the old answer - a stdio client that is not VS Code was not an
   * "MCP environment" by that reading, which is exactly how `MCP_TRANSPORT=stdio` came to
   * be ignored here while `index.js` and `cli.js` honoured it.
   *
   * The judgement itself lives in `mcp-environment.js` so that this class and those two
   * entry points cannot drift apart again. This thin wrapper remains because it is the
   * seam the logger tests stub.
   *
   * @returns {boolean} True when console output must go to stderr
   * @private
   */
  _isMcpStdioTransport() {
    return isMcpStdioTransport();
  }
