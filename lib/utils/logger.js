...
/**
 * Every level routed to stderr, unconditionally.
 *
 * **There is no detection here on purpose** (#1260). This Logger is constructed in exactly
 * one place - `index.js`, the MCP server - and that server's only transport is
 * `StdioServerTransport`. So stdout is *always* the JSON-RPC channel, for every client,
 * and console output always belongs on stderr. Asking "are we under an MCP client?" is a
 * question with one answer; the old predicate could only ever get it wrong (it did, twice:
 * #1256, #1259). Please do not re-introduce it.
 *
...
export class Logger {
  ...
  createLogger() {
    ...
    if (process.env.NODE_ENV !== 'production') {
      // No colors, ever. This process is always a spawned stdio server (see STDERR_LEVELS
      // above), so its console output is always something a client renders in a log pane
      // rather than a terminal that understands ANSI. There used to be a colorized variant
      // selected by transport detection; with the detection gone the colorized branch was
      // unreachable, so it went with it (#1260).
      formats.push(
    ...
    if (process.env.NODE_ENV !== 'test' || process.env.ENABLE_TEST_LOGGING === 'true') {
      const consoleTransportOptions = {
        ...
        // Every level to stderr, with nothing to decide: stdout carries JSON-RPC whenever
        // this Logger exists at all. See STDERR_LEVELS above for why there is no `if`.
  ...
  createSecurityLogger() {
    ...
    if (process.env.NODE_ENV !== 'test' || process.env.ENABLE_TEST_LOGGING === 'true') {
      const consoleTransportOptions = {
        level: 'info',
        // The audit channel gets the same unconditional treatment as the main logger,
        // rather than being dropped: an audit line that silently disappears is its own
        // hazard - with no SECURITY_LOG_FILE set, the console is the only record of a
        // blocked query - so this moves the record, it does not discard it. No detection
        // here either, for the reason given at STDERR_LEVELS.
        stderrLevels: [...STDERR_LEVELS],
