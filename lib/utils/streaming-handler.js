 ...
 */
/**
 * CSV row terminator. Written as a real newline: the literal `'\\n'` that stood here until
 * 2026-09-15 is a TWO-CHARACTER string in JavaScript (backslash, n), so every export was a
 * single line with `\\n` printed between the rows and no CSV reader could parse it.
 */
const CSV_ROW_TERMINATOR = '\n';

/** Fields containing a comma, a double quote, CR or LF must be quoted (RFC 4180). */
...
export class StreamingHandler {
    ...
    if (context.tableName && context.schema) {
      try {
        const sizeQuery = `
          ...
          WHERE t.name = '${escapeSqlStringLiteral(context.tableName)}'
            AND s.name = '${escapeSqlStringLiteral(context.schema)}'
            AND p.index_id <= 1
        `;
  ...
  batchToCsv(batch, context) {
    ...
    if (!context.csvHeaderAdded) {
      ...
      csvContent += headers.join(',') + CSV_ROW_TERMINATOR;
    ...
    for (const row of batch) {
      const values = Object.values(row).map(value => {
        ...
        // RFC 4180: a field must be quoted if it contains the delimiter, a quote, or a
        // line break, and an embedded quote is doubled. CR is checked as well as LF - a
        // lone CR inside an unquoted field breaks the row for readers that accept CR,
        // CRLF and LF as terminators.
        if (CSV_MUST_QUOTE.test(stringValue)) {
          return `"${stringValue.replace(/"/g, '""')}"`;
        }
        return stringValue;
      });

      csvContent += values.join(',') + CSV_ROW_TERMINATOR;
    }
  ...
  async streamTableExport(request, tableName, options = {}) {
    ...
    // Coerce the pagination value defensively: callers should already pass a
    // validated integer, but streamTableExport may be invoked directly. A falsy
    // limit (0/null/undefined) means "no TOP clause / unbounded"; only a truthy
    // value is validated as an integer >= 1.
    const safeLimit = limit
      ? parseRowCount(limit, { name: 'limit', min: 1, fallback: null })
      : null;
    const topClause = safeLimit ? ` TOP ${safeLimit}` : '';
    ...
    let query = `SELECT${topClause} * FROM [${safeSchema}].[${safeTableName}]`;
  ...
  getStreamingStats(streamingResult) {
    if (!streamingResult.streaming) {
      // executeRegularQuery() reports the count at performance.rowCount; an
      // explicit rowCount or the recordset length are accepted for callers
      // that hand in a plain driver result.
      return {
        streaming: false,
        memoryEfficient: false,
        totalRows:
          streamingResult.rowCount ??
          streamingResult.performance?.rowCount ??
          streamingResult.recordset?.length ??
          0
      };
    }
  ...
  _safeJsonParse(jsonString) {
    ...
    try {
      ...
      throw new Error(`JSON parsing failed: ${error.message}`, { cause: error });
    }
