...
import { countFormulaRiskyCells, csvRow } from './csv.js';
import {
...
export class StreamingHandler {
  ...
  processBatch(batch, chunks, chunkNumber, context) {
    ...
    switch (context.outputFormat) {
      case 'csv':
        // Counted BEFORE batchToCsv, which flips context.csvHeaderAdded. The header row is
        // written once, by whichever batch happens to be first, and a spreadsheet evaluates a
        // column name exactly like a data cell - so the headers are only in scope here.
        riskyCells = countFormulaRiskyCells(batch, !context.csvHeaderAdded);
    ...
    chunks.push({
      ...
      // Rides out on the chunk because `chunks` is what streamTableExport returns; the
      // processing context is rebuilt per call and never reaches the caller. Always 0 for
      // JSON and raw, which nobody opens in a spreadsheet.
      riskyCells
    });
  }
  ...
  batchToCsv(batch, context) {
    ...
    if (!context.csvHeaderAdded) {
      csvContent += csvRow(Object.keys(batch[0]));
      context.csvHeaderAdded = true;
    }

    // Add data rows
    for (const row of batch) {
      csvContent += csvRow(Object.values(row));
    }
