...
function logConnectionDebug(message) {
  if (process.env.NODE_ENV !== 'test' && process.env.SQL_SERVER_DEBUG === 'true') {
    // Evaluate messages only when logging is enabled, including error.message reads.
    console.error(message());
  }
}

function waitForConnectionRetry(retryDelay, attempt) {
...
export class ConnectionManager {
  ...
  async connect() {
    ...
    logConnectionDebug(() => 'Establishing new database connection...');
    ...
    while (attempt < this.maxRetries) {
      try {
        ...
        logConnectionDebug(() => `Connected to SQL Server successfully (attempt ${attempt})`);
        return this.pool;
      } catch (error) {
        lastError = error;
        logConnectionDebug(() => `Connection attempt ${attempt} failed: ${error.message}`);
        if (attempt >= this.maxRetries) break;

        await waitForConnectionRetry(this.retryDelay, attempt);
      }
    }
  ...
  getConnectionHealth() {
    ...
    const health = {
      ...
      pool: {
        ...
        // Capacity: the tarn pool's resolved max, falling back to the configured value
        max: this.pool.pool?.max ?? this.pool.config?.pool?.max
      }
    };
