...
- **Total Tests**: 1,754 - **every one of them runs automatically on every pull request**
  - 1,687 unit tests (`npm run test:unit`, 39 files, mocked)
...
| ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------ |
| `test/unit/**`                             | 1,687           | `npm run test:unit`                    | Yes - required `Tests (22)` / `(24)`       |
...
The 1,687 unit tests are spread across 39 files. Grouped by area, the totals sum to 1,687:
...
| Repository and CLI tooling      | 618   | `cleanup-test-processes` (48), `release-script` (145), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (43), `packed-window` (25), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6), `check-orphan-docs` (71), `markdown-blocks` (21), `check-env-var-docs` (103), `check-tool-docs` (11), `run-doc-checks` (3), `cli` (6), `docker-command-utils` (4), `dependabot-config` (2) |
