...
- **Total Tests**: 1,676 - **every one of them runs automatically on every pull request**
  - 1,609 unit tests (`npm run test:unit`, 36 files, mocked)
...
| ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------ |
| `test/unit/**`                             | 1,609           | `npm run test:unit`                    | Yes - required `Tests (22)` / `(24)`       |
...
The 1,609 unit tests are spread across 36 files. Grouped by area, the totals sum to 1,609:
...
| Repository and CLI tooling      | 566   | `cleanup-test-processes` (48), `release-script` (145), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (40), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6), `check-orphan-docs` (71), `markdown-blocks` (17), `check-env-var-docs` (99), `cli` (4), `docker-command-utils` (4), `dependabot-config` (2) |
