...
- **Total Tests**: 1,398 - **every one of them runs automatically on every pull request**
  - 1,331 unit tests (`npm run test:unit`, 30 files, mocked)
...
| ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------ |
| `test/unit/**`                             | 1,331           | `npm run test:unit`                    | Yes - required `Tests (22)` / `(24)`       |
...
The 1,331 unit tests are spread across 30 files. Grouped by area, the totals sum to 1,331:
...
| Repository and CLI tooling      | 310   | `cleanup-test-processes` (48), `release-script` (105), `git-spawn-scrub-guard` (50), `verify-publish-tree` (40), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6), `cli` (4), `docker-command-utils` (4), `dependabot-config` (2) |
