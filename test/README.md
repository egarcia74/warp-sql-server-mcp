...
- **Total Tests**: 1,580 - **every one of them runs automatically on every pull request**
  - 1,513 unit tests (`npm run test:unit`, 35 files, mocked)
...
| ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------ |
| `test/unit/**`                             | 1,513           | `npm run test:unit`                    | Yes - required `Tests (22)` / `(24)`       |
...
The 1,513 unit tests are spread across 35 files. Grouped by area, the totals sum to 1,513:
...
| Repository and CLI tooling      | 470   | `cleanup-test-processes` (48), `release-script` (145), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (40), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6), `check-orphan-docs` (43), `check-env-var-docs` (48), `cli` (4), `docker-command-utils` (4), `dependabot-config` (2) |
