...
- **Total Tests**: 1,199 - **every one of them runs automatically on every pull request**
  - 1,132 unit tests (`npm run test:unit`, 27 files, mocked)
...
The 1,132 unit tests are spread across 27 files. Grouped by area, the totals sum to 1,132:
...
| Repository and CLI tooling      | 83    | `cleanup-test-processes` (48), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6), `cli` (4), `docker-command-utils` (4), `dependabot-config` (2)            |
