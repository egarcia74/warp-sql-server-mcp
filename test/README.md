...
- **Total Tests**: 1,386 - **every one of them runs automatically on every pull request**
  - 1,319 unit tests (`npm run test:unit`, 30 files, mocked)
...
| ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------ |
| `test/unit/**`                             | 1,319           | `npm run test:unit`                    | Yes - required `Tests (22)` / `(24)`       |
...
The 1,319 unit tests are spread across 30 files. Grouped by area, the totals sum to 1,319:
...
| Repository and CLI tooling      | 298   | `cleanup-test-processes` (48), `release-script` (105), `git-spawn-scrub-guard` (38), `verify-publish-tree` (40), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6), `cli` (4), `docker-command-utils` (4), `dependabot-config` (2) |
