...
- **Total Tests**: 1,197 - **every one of them runs automatically on every pull request**
  - 1,130 unit tests (`npm run test:unit`, 27 files, mocked)
...
| ------------------------------------------ | --------------- | -------------------------------------- | ------------------------------------------ |
| `test/unit/**`                             | 1,130           | `npm run test:unit`                    | Yes - required `Tests (20)` / `(22)`       |
...
The 1,130 unit tests are spread across 27 files. Grouped by area, the totals sum to 1,130:
...
| Repository and CLI tooling      | 81    | `cleanup-test-processes` (48), `docs-html-writer` (8), `check-fenced-blocks` (9), `link-checker` (6), `cli` (4), `docker-command-utils` (4), `dependabot-config` (2)             |
