...
| **Security level detection**                 | `ServerConfig.getSecurityConfig()`, `SqlServerMCP.validateQuery` + `lib/security/sql-batch-guard.js`, `ServerConfig._analyzeEnvironment()` | No recommendation engine that compares posture to environment.                        |
