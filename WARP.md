...
**✅ Production Status**: This MCP server has been **fully validated** through 1,197 tests, every
one of which runs automatically on every pull request (1,130 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,197 tests total, **all of them automated** on every pull request:
  1,130 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(27 files, 1,130 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 27 files, 1,130 tests
...
│   ├── docs-html-writer.test.js              #   8 - Docs writer: timestamp churn guard
...
#### **Unit Tests (1,130 across 27 files)**

Grouped by area; the group totals sum to 1,130:
...
- **Repository and CLI tooling** (81): `cleanup-test-processes` (48), `docs-html-writer` (8), `check-fenced-blocks` (9),
...
>   suite has since grown to 1,130 automated unit tests)
...
During intensive testing sessions (like our 1,130-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
