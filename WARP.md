...
**✅ Production Status**: This MCP server has been **fully validated** through 1,456 tests, every
one of which runs automatically on every pull request (1,389 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,456 tests total, **all of them automated** on every pull request:
  1,389 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(31 files, 1,389 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 31 files, 1,389 tests
...
│   ├── release-script.test.js               # 134 - Release dispatch: type detection, semver bump, flags, run selection
...
#### **Unit Tests (1,389 across 31 files)**

Grouped by area; the group totals sum to 1,389:
...
- **Repository and CLI tooling** (368): `cleanup-test-processes` (48), `release-script` (134), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (40),
...
>   suite has since grown to 1,389 automated unit tests)
...
During intensive testing sessions (like our 1,389-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
