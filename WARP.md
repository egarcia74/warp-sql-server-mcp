...
**✅ Production Status**: This MCP server has been **fully validated** through 1,455 tests, every
one of which runs automatically on every pull request (1,388 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,455 tests total, **all of them automated** on every pull request:
  1,388 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(31 files, 1,388 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 31 files, 1,388 tests
...
#### **Unit Tests (1,388 across 31 files)**

Grouped by area; the group totals sum to 1,388:
...
- **Repository and CLI tooling** (367): `cleanup-test-processes` (48), `release-script` (133), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (40),
...
>   suite has since grown to 1,388 automated unit tests)
...
During intensive testing sessions (like our 1,388-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
