...
**✅ Production Status**: This MCP server has been **fully validated** through 1,761 tests, every
one of which runs automatically on every pull request (1,694 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,761 tests total, **all of them automated** on every pull request:
  1,694 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(39 files, 1,694 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 39 files, 1,694 tests
...
│   ├── performance-monitor.test.js          #  84 - Metrics collection
│   ├── streaming-handler.test.js            #  63 - Streaming large result sets
│   ├── tool-registry.test.js                #  59 - Tool definitions and input schemas
│   ├── database-tools-handler.test.js       #  59 - Database tool handlers
│   ├── logger.test.js                       #  56 - Structured logging and audit trail
│   ├── mcp-security.test.js                 #  38 - Three-tier safety system
│   ├── connection-manager.test.js           #  34 - Pooling, retries, auth
...
│   ├── release-script.test.js               # 146 - Release dispatch: type detection, semver bump, flags, run selection
...
│   ├── csv.test.js                              #   13 - CSV serialisation shared by both export paths
│   ├── result-formatter.test.js                 #   7 - Text-table rendering shared by every tool
│   ├── link-checker.test.js                 #   8 - Documentation link validation
...
#### **Unit Tests (1,694 across 39 files)**

Grouped by area; the group totals sum to 1,694:
...
- **SQL safety and injection guards** (317): `sql-injection-battery` (103), `where-clause-guard` (81),
  ...
  `sql-identifier` (16)
- **Infrastructure utilities** (274): `csv` (26), `result-formatter` (7), `performance-monitor` (84), `streaming-handler` (67),
  `logger` (56), `connection-manager` (34)
- **Query analysis** (158): `query-optimizer` (136), `query-optimizer-security` (13),
  `bottleneck-detector` (9)
- **Tools and handlers** (152): `tool-registry` (59), `database-tools-handler` (64),
  `get-server-info` (29)
- **Configuration** (24): `server-config` (24)
- **Repository and CLI tooling** (625): `cleanup-test-processes` (48), `release-script` (146), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (43),
  `packed-window` (29), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (8),
...
>   suite has since grown to 1,694 automated unit tests)
...
During intensive testing sessions (like our 1,694-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
...
The preferred way to trigger it is the release script, which shows what the workflow will do
before anything is tagged:

```bash
npm run release:dry     # preview + dispatch with dry_run=true; nothing is tagged
npm run release         # preview, type the version to confirm, dispatch, watch
```

`scripts/release.mjs` (the CLI; its decision rules are the pure functions in
`scripts/lib/release-plan.mjs`) checks that `gh` is authenticated, pins every `gh` call to the
repository that remote `origin` names (`--repo owner/name`, so `GH_REPO` or `gh repo set-default`
cannot point the dispatch elsewhere), unshallows a shallow clone, fetches `origin/main` by explicit
refspec, reads tags from the remote (`git ls-remote`) so a stale local tag cannot skew the version,
...
- Auto detection (when `release_type=auto`) — **the subjects that count are whatever lands on
  ...
  | `test:` / `ci:` / `style:`                                        | patch                                 |
  | `deps:` / `deps-dev:` (Dependabot's own prefixes)                 | patch                                 |
  | anything else (an unprefixed subject, a malformed one, …)         | **none** — and logged as unclassified |
  ...
  patch — provided the window changes something npm packs.** Since #1235 the workflow also
  intersects the window's changed paths with npm's own packlist and refuses a window that
  reaches nothing in the tarball, whatever its subjects say; so "no recognised type" is no
  longer the only non-releasing outcome. The rule lives in `scripts/lib/packed-window.mjs`
  and is reported as `Ships to npm:` by `npm run release`. On the subject side,
  `test:`, `ci:` and `style:` are included: a commit-type prefix is a label the PR
  ...
  npm packs (#1235, reported as `skip_reason=no-packed-changes`). An explicit
  `release_type=patch` (or `npm run release -- --type patch`) forces a release over the **second
  and third** — it bypasses the path gate as well as the subject rules. It cannot force one over
  an **empty** window: that branch returns before the forced type is applied, because there is
  nothing new to tag.
  ...
  are in the table because 739 of this repository's commits carry one of them, so a window
  of nothing but dependency bumps used to compute `none` and ship nothing (#1232). Both are
  patch, `deps-dev:` included: `package.json` is always in the npm tarball and carries
  `devDependencies` verbatim, so a dev-dependency bump changes the published bytes just as a
  runtime one does. Mapping `deps-dev:` to `none` would make it a _recognised_ yet
  non-releasing type, which would silence the unclassified warning above and send a
  dependency-only window back to shipping nothing silently. Replayed over the 26 windows
  `git describe` actually produces (29 `v*` tags, 27 reachable from `main`, less `v1.2.0` which has
  no base - an earlier note said "all 30 `v*` tag windows", counting a non-`v` tag and the
  unreachable ones), these two rules change **no** past window's release level.
