...
**✅ Production Status**: This MCP server has been **fully validated** through 1,757 tests, every
one of which runs automatically on every pull request (1,690 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,757 tests total, **all of them automated** on every pull request:
  1,690 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(39 files, 1,690 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 39 files, 1,690 tests
...
#### **Unit Tests (1,690 across 39 files)**

Grouped by area; the group totals sum to 1,690:
...
- **Repository and CLI tooling** (621): `cleanup-test-processes` (48), `release-script` (145), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (43),
  `packed-window` (28), `classify-dependabot-pr` (32), `docs-html-writer` (10), `check-fenced-blocks` (9), `link-checker` (6),
...
>   suite has since grown to 1,690 automated unit tests)
...
During intensive testing sessions (like our 1,690-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
