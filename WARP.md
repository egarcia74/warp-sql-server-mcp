...
**✅ Production Status**: This MCP server has been **fully validated** through 1,180 tests, every
one of which runs automatically on every pull request (1,113 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,180 tests total, **all of them automated** on every pull request:
  1,113 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(26 files, 1,113 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 26 files, 1,113 tests
...
│   ├── cleanup-test-processes.test.js        #  39 - Process inspector: PID guards, exit status
...
#### **Unit Tests (1,113 across 26 files)**

Grouped by area; the group totals sum to 1,113:
...
- **Repository and CLI tooling** (64): `cleanup-test-processes` (39), `check-fenced-blocks` (9),
...
>   suite has since grown to 1,113 automated unit tests)
...
During intensive testing sessions (like our 1,113-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
