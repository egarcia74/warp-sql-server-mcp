...
**✅ Production Status**: This MCP server has been **fully validated** through 1,290 tests, every
one of which runs automatically on every pull request (1,223 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,290 tests total, **all of them automated** on every pull request:
  1,223 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(29 files, 1,223 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 29 files, 1,223 tests
...
│   ├── streaming-handler.test.js            #  61 - Streaming large result sets
...
│   ├── release-script.test.js               #  47 - Release dispatch: type detection, semver bump, flags, run selection
...
#### **Unit Tests (1,223 across 29 files)**

Grouped by area; the group totals sum to 1,223:
...
- **SQL safety and injection guards** (317): `sql-injection-battery` (103), `where-clause-guard` (81),
  ...
  `sql-identifier` (16)
- **Infrastructure utilities** (231): `performance-monitor` (84), `streaming-handler` (61),
...
- **Repository and CLI tooling** (202): `cleanup-test-processes` (48), `release-script` (47), `verify-publish-tree` (40),
...
>   suite has since grown to 1,223 automated unit tests)
...
During intensive testing sessions (like our 1,223-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
...
`scripts/lib/release-plan.mjs`) checks that `gh` is authenticated, fetches `origin/main`, warns if the
local `main` is not at `origin/main` (the workflow releases `origin/main`'s HEAD, and the
...
only when you type the previewed version back exactly; it dispatches with `--ref main`, finds
the run it started, watches it, and prints the Release URL, the `chore/release/vX.Y.Z` bump
PR and the remaining steps (CHANGELOG heading, merge, verify on npm). The workflow still makes
