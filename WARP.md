...
**✅ Production Status**: This MCP server has been **fully validated** through 1,195 tests, every
one of which runs automatically on every pull request (1,128 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,195 tests total, **all of them automated** on every pull request:
  1,128 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(27 files, 1,128 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 27 files, 1,128 tests
...
│   ├── docs-html-writer.test.js              #   6 - Docs writer: timestamp churn guard
...
#### **Unit Tests (1,128 across 27 files)**

Grouped by area; the group totals sum to 1,128:
...
- **SQL safety and injection guards** (317): `sql-injection-battery` (103), `where-clause-guard` (81),
  ...
  `sql-identifier` (16)
- **Infrastructure utilities** (215): `performance-monitor` (70), `streaming-handler` (60),
...
- **Repository and CLI tooling** (79): `cleanup-test-processes` (48), `docs-html-writer` (6), `check-fenced-blocks` (9),
...
>   suite has since grown to 1,128 automated unit tests)
...
During intensive testing sessions (like our 1,128-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
