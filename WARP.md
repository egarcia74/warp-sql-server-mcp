...
**✅ Production Status**: This MCP server has been **fully validated** through 1,489 tests, every
one of which runs automatically on every pull request (1,422 unit + 27 integration + 40
...
- **Comprehensive Coverage**: 1,489 tests total, **all of them automated** on every pull request:
  1,422 unit and 27 integration under Vitest, plus the 40 live-database phase tests that `npm test`
...
(33 files, 1,422 tests); the live-database suites are counted from their own runners.
...
├── unit/                                    # Vitest unit suites - 33 files, 1,422 tests
...
#### **Unit Tests (1,422 across 33 files)**

Grouped by area; the group totals sum to 1,422:
...
- **Repository and CLI tooling** (379): `cleanup-test-processes` (48), `release-script` (145), `git-spawn-scrub-guard` (50), `run-one-test` (29), `verify-publish-tree` (40),
...
>   suite has since grown to 1,422 automated unit tests)
...
During intensive testing sessions (like our 1,422-test unit suite), Node.js/Vitest processes can sometimes become orphaned and consume significant system resources.
